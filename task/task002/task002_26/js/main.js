...
function mediator(uCommand) {
    if (Math.random() > 0.3) {
        //发送成功
        var len = airshipArr.length;


        if (uCommand.command == 'create') {
            setTimeout(function() {
                airshipArr[uCommand.id] = new Airship(uCommand.id);
            }, 1000);
        }
...
function Airship(id) {
    ...
    this.receive = function(uCommand) {
        ...
        if (isMe) {
            this.orbit = uCommand.id;
            this.dynamical = uCommand.dynamical;
            this.energy = uCommand.energy;
            this.useEnergy = uCommand.useEnergy;
